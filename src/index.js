  ...
  $http.get(ROOT_URL)
    .then(function(res){
      if(res.status > 199 && res.status < 300 && !res.data.items[0]){
        $location.path("/");
        alert("User exist but no repositories has been created yet, please try another username!");
      }
...
/*Second $http for /:user/:index route to get readme.md within specific repository*/
  $scope.fetchRM = function(user, $index){
    ...
    .catch(function(err){
      $location.path("/")
      alert("This repository does not have a README.md file, try another username instead?");
      console.error(err)
    })
  }
